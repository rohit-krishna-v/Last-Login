...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#030303] text-white overflow-hidden selection:bg-white/10 relative">
      <Scene3D accentColor="#ffffff" />
      ...
      <section className="relative z-10 container mx-auto px-6 py-32 min-h-screen flex flex-col items-center justify-center text-center">
        ...
        >
          ...
          <SplitText 
            ...
            className="text-xl md:text-3xl text-gray-400 font-light italic mb-16"
            delay={0.8}
          />
        </motion.div>
        ...
        <motion.div
          ...
          className="absolute bottom-12 flex flex-col items-center hidden md:flex"
        >
        ...
        <div className="max-w-4xl mx-auto space-y-16 md:space-y-24">
          {[
            ...
            <div key={i} className="memory-item opacity-0 flex flex-col md:flex-row gap-4 md:gap-8 items-start group">
              ...
              <div className="flex-1 glass p-6 md:p-8 rounded-2xl relative overflow-hidden w-full">
      ...
      {/* Faculty Invitation Directory */}
      <section className="relative z-10 py-24 md:py-32 container mx-auto px-6 border-t border-white/5 min-h-screen flex flex-col justify-center">
        <div className="text-center mb-16 md:mb-24">
          <h2 className="text-3xl md:text-6xl font-bold mb-4 tracking-tight">Admin Access</h2>
          <p className="text-gray-400 text-base md:text-lg font-light max-w-2xl mx-auto">Personalized access codes for our esteemed faculty members.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl mx-auto">
          {facultyData.map((faculty, i) => (
            <Link 
              key={faculty.id} 
              to={`/invite/${faculty.id}`}
              className="group relative rounded-2xl overflow-hidden hover:scale-[1.02] transition-transform duration-500 ease-out h-[400px]"
            >
              ...
              />
              <div className={`absolute inset-0 bg-gradient-to-br ${faculty.theme.accent} opacity-10 group-hover:opacity-40 transition-opacity duration-500 z-10 mix-blend-overlay`} />
              
              <div className="relative h-full p-6 flex flex-col justify-end text-left z-20">
                <h3 className="text-2xl font-semibold text-white drop-shadow-lg mb-1 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                  {faculty.name}
                </h3>
                ...
                <div className="mt-4 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 delay-200">
                  <span className={`text-xs font-medium uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r ${faculty.theme.accent}`}>
                    Access Invite
                  </span>
